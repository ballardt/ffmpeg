...
CG_ENTER
CG_IN("AVOutputFormat known? %s", (oformat?"yes":"no"))
CG_IN("format string: %s", (format?format:"<unknown>"))
CG_IN("filename: %s", (filename?filename:"<unknown>"))
CG_IN("calling avformat_alloc_context")
...
CG_IN("guessing format from filename: %s", (filename?filename:"<unknown>"))
...
CG_IN("adding oformat to new context")
    s->oformat = oformat;
    if (s->oformat->priv_data_size > 0) {
CG_IN("oformat has private data")
        s->priv_data = av_mallocz(s->oformat->priv_data_size);
        if (!s->priv_data)
            goto nomem;
        if (s->oformat->priv_class) {
            *(const AVClass**)s->priv_data= s->oformat->priv_class;
CG_IN("set default options in private data")
...
CG_IN("adding filename %s to new context", filename);
...
CG_IN("num streams %d", s->nb_streams)
    // some sanity checks
